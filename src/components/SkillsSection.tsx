...
const marqueeTools = [
  "Figma", "Adobe XD", "Photoshop", "Illustrator", "Sketch", "CorelDRAW",
  "HTML5", "CSS3", "SCSS", "TailwindCSS", "Bootstrap", "UIkit",
  "JavaScript", "ReactJS", "jQuery", "Git", "Bitbucket", "Jira",
];
  ...
  return (
    <section id="skills" className="section-padding relative" ref={ref}>
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        {/* Auto-scrolling tool marquee */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={inView ? { opacity: 1 } : {}}
          transition={{ duration: 0.6, delay: 0.15 }}
          aria-hidden
          className="marquee-mask overflow-hidden mb-12 md:mb-16"
        >
          <div className="flex w-max gap-3 animate-marquee hover:[animation-play-state:paused]">
            {[...marqueeTools, ...marqueeTools].map((tool, i) => (
              <span
                key={`${tool}-${i}`}
                className="shrink-0 glass-card px-5 py-2.5 text-sm font-medium text-foreground/80 border-white/10"
              >
                {tool}
              </span>
            ))}
